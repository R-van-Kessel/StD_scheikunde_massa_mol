...
const SUBTITLE_TEXT = 'Gebruik je binas en los de sommen op om de draak op te roepen!';
...
function generateQuestions() {
    ...
    // ===== STOFFEN DATABASE MET MOLMASSA =====
    const substances = [
        { name: 'H₂O',    molarMass: 18.0  },
        { name: 'NaCl',   molarMass: 58.5  },
        { name: 'CO₂',    molarMass: 44.0  },
        { name: 'O₂',     molarMass: 32.0  },
        { name: 'N₂',     molarMass: 28.0  },
        { name: 'H₂',     molarMass: 2.0   },
        { name: 'Cl₂',    molarMass: 71.0  },
        { name: 'HCl',    molarMass: 36.5  },
        { name: 'NH₃',    molarMass: 17.0  },
        { name: 'CH₄',    molarMass: 16.0  },
        { name: 'SO₂',    molarMass: 64.0  },
        { name: 'SO₃',    molarMass: 80.0  },
        { name: 'NO₂',    molarMass: 46.0  },
        { name: 'HNO₃',   molarMass: 63.0  },
        { name: 'H₂SO₄',  molarMass: 98.0  },
        { name: 'NaOH',   molarMass: 40.0  },
        { name: 'KOH',    molarMass: 56.0  },
        { name: 'CaCO₃',  molarMass: 100.0 },
        { name: 'CaO',    molarMass: 56.0  },
        { name: 'Ca(OH)₂',molarMass: 74.0  },
        { name: 'MgO',    molarMass: 40.0  },
        { name: 'Al₂O₃',  molarMass: 102.0 },
        { name: 'FeO',    molarMass: 71.8  },
        { name: 'Fe₂O₃',  molarMass: 159.7 },
        { name: 'CuO',    molarMass: 79.5  },
        { name: 'CuSO₄',  molarMass: 159.6 },
        { name: 'ZnO',    molarMass: 81.4  },
        { name: 'KCl',    molarMass: 74.5  },
        { name: 'Na₂CO₃', molarMass: 106.0 },
        { name: 'K₂SO₄',  molarMass: 174.3 },
        { name: 'C₆H₁₂O₆',molarMass: 180.2 }, // glucose
        { name: 'C₂H₅OH', molarMass: 46.1  }, // ethanol
        { name: 'C₁₂H₂₂O₁₁', molarMass: 342.3 }, // sacharose
    ];

    // Kies 10 willekeurige unieke stoffen
    let shuffled = shuffle([...substances]);
    let selected = shuffled.slice(0, 10);

    let questions = [];
    let answerValues = [];

    for (let sub of selected) {
        let questionType = random() < 0.5 ? 'gram_to_mol' : 'mol_to_gram';
        let questionText, answerValue;

        if (questionType === 'gram_to_mol') {
            // Kies een massa die een "nette" hoeveelheid mol geeft
            // Genereer een mol-waarde en reken terug naar gram
            let molOptions = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 0.25, 0.1, 0.2];
            let molChosen = random(molOptions);
            let gramValue = Math.round(molChosen * sub.molarMass * 10) / 10;

            questionText = gramValue + ' g\n' + sub.name + '\n→ mol?';
            answerValue = parseFloat((gramValue / sub.molarMass).toFixed(1));

        } else {
            // mol → gram
            let molOptions = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 0.1, 0.2, 0.25, 4.0, 5.0];
            let molChosen = random(molOptions);
            molChosen = parseFloat(molChosen.toFixed(1));
            let gramValue = parseFloat((molChosen * sub.molarMass).toFixed(1));

            questionText = molChosen + ' mol\n' + sub.name + '\n→ gram?';
            answerValue = gramValue;
        }

        questions.push({ text: questionText, answer: answerValue });
        answerValues.push(answerValue);
    }

    // Shuffle antwoorden voor de antwoordblokjes
    let shuffledAnswers = shuffle([...answerValues]);
    ...
    for (let row = 3; row < 5; row++) {
        for (let col = 0; col < COLS; col++) {
            blocks.push({
                ...
                answer: shuffledAnswers[answerIndex],
                isQuestion: false,
                isCorrect: null,
                isHovered: false,
                hoverProgress: 0,
